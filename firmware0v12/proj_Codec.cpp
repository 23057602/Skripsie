...
char codec_heartbeat(){//returns @ char if alive
...
void codec_setup(){
  ...
  val = 0xE0;//11.2896MHz to 44.1kHz
  i2c_write(i2c_address, regist, val);
  ...
  //regist = 0x50;//clock enables
  //val = 0x0F;//enable dac clocks
  //i2c_write(i2c_address, regist, val);
  ...
  //Signal path setup
  regist = 0x22;//DAC mix
  val = 0x84;//right to right,left to left
  i2c_write(i2c_address, regist, val);
  ...
  val = 0xC3;//left and right enable
  i2c_write(i2c_address, regist, val);
  ...
  //regist = 0x0F;//interrupt enable
  //val = 0xA0;//clipping and port errors
  //i2c_write(i2c_address, regist, val);

  regist = 0x31;//Anc gain
  val = 0x0B;//left and right enable
  i2c_write(i2c_address, regist, val);

  regist = 0x18;//filters
  val = 0x80;//use music filter
  i2c_write(i2c_address, regist, val);
}
